...
public class LogDL extends Log{
    ...
    protected void analyseLog() {
        ...
        //1. Move all the log lines with 206 status to a seperate array
            ...
            List<LogLine> partialHits = this.logLines.stream().filter(x -> x.getOutputs().get("status").equals("206")).collect(Collectors.toList());
            List<LogLine> fullHits = this.logLines.stream().filter(x -> !(x.getOutputs().get("status").equals("206"))).collect(Collectors.toList());
        ...
        //First create a map of all the distinct url_ipadrress_ua combinations. This is an original hit.
            Map<String,LogLine> sessionLines = new HashMap<>();
            for(LogLine line : partialHits) {
